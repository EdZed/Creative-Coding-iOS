...
//  CatNap
//
//  Created by Edgar Castro on 3/11/14.
...
{
    //contains all objects in current level
    SKNode *_gameNode;
    
    //these two are cat and bed sprite nodes
    SKSpriteNode *_catNode;
    SKSpriteNode *_bedNode;
    
    //helper instance variable, keeps track of level player is currently on
    int _currentLevel;
...
//takes in list of catapult locations and creates sprites
...
//method stub to initialize hook structure
...
{
    //base
    SKSpriteNode* seesawBase = [SKSpriteNode spriteNodeWithImageNamed:@"45x45"];
    seesawBase.position = position;
    seesawBase.physicsBody = [SKPhysicsBody bodyWithRectangleOfSize: seesawBase.size];
    seesawBase.physicsBody.collisionBitMask = kNilOptions;
    seesawBase.physicsBody.categoryBitMask = kNilOptions;
    [_gameNode addChild: seesawBase];
    
    SKPhysicsJointFixed* fixJoint = [SKPhysicsJointFixed jointWithBodyA: seesawBase.physicsBody
    ...
    SKPhysicsJointPin *pin = [SKPhysicsJointPin jointWithBodyA:seesawBase.physicsBody
