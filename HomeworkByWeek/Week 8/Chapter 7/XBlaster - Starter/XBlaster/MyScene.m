...
- (void)setupSceneLayers
{
    ...
    [starfieldNode addChild:[self starfieldEmitterNodeWithSpeed:-10 lifetime:(self.frame.size.height / 15) scale:0.10 birthRate:5 color:[SKColor darkGrayColor]]];
    
    [starfieldNode addChild:[self starfieldEmitterNodeWithSpeed:-16 lifetime:(self.frame.size.height / 10) scale:0.14 birthRate:2 color:[SKColor grayColor]]];
    
    [starfieldNode addChild:[self starfieldEmitterNodeWithSpeed:-24 lifetime:(self.frame.size.height / 23) scale:0.2 birthRate:1 color:[SKColor lightGrayColor]]];
...
//for particles
- (SKEmitterNode *)starfieldEmitterNodeWithSpeed:(float)speed lifetime:(float)lifetime scale:(float)scale birthRate:(float)birthRate color:(SKColor*)color{
    ...
    // 1
    float twinkles = 20;
    SKKeyframeSequence *colorSequence = [[SKKeyframeSequence alloc] initWithCapacity:twinkles*2]; // 2
    float twinkleTime = 1.0/twinkles; for (int i = 0; i < twinkles; i++) {
        // 3
