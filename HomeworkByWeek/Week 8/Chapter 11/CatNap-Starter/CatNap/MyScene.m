...
- (void)initializeScene
{
  ...
  [self addChild: bg];
    ...
    SKLabelNode *label = [SKLabelNode labelNodeWithFontNamed:@"Zapfino"]; label.text = @"Cat Nap";
    label.fontSize = 96;
    SKCropNode *cropNode = [SKCropNode node];
    [cropNode addChild:bg2];
    [cropNode setMaskNode:label];
    cropNode.position = CGPointMake(self.size.width/2, self.size.height/2); [self addChild:cropNode];
  ...
  for (NSDictionary *block in blocks) {
    ...
    if (block[@"tuple"]) {
      ...
      [_gameNode addChild:blockSprite];
          ...
          else if ([blockType isEqualToString:@"wonlyBlock"]) {
...
CGPoint adjustedPoint(CGPoint inputPoint, CGSize inputSize) {
    //1
    float width = inputSize.width * .15; float height = inputSize.height * .15;
    //2
    float xMove = width * RandomFloat() - width / 2.0; float yMove = height * RandomFloat() - height / 2.0;
    //3
...
- (SKShapeNode *)createWonkyBlockFromRect:(CGRect)inputRect {
    ...
    CGPointMake(inputRect.origin.x - inputRect.size.width / 2.0,
                inputRect.origin.y - inputRect.size.height/2.0); CGPoint pointlb = origin;
    CGPoint pointlt = CGPointMake(origin.x,
                                  inputRect.origin.y + inputRect.size.height); CGPoint pointrb =
    CGPointMake(origin.x + inputRect.size.width, origin.y); CGPoint pointrt =
    CGPointMake(origin.x + inputRect.size.width, origin.y + inputRect.size.height);
    //2
    pointlb = adjustedPoint(pointlb, inputRect.size); pointlt = adjustedPoint(pointlt, inputRect.size); pointrb = adjustedPoint(pointrb, inputRect.size); pointrt = adjustedPoint(pointrt, inputRect.size);
    //3
    UIBezierPath *shapeNodePath = [UIBezierPath bezierPath]; [shapeNodePath moveToPoint:pointlb];
    [shapeNodePath addLineToPoint:pointlt]; [shapeNodePath addLineToPoint:pointrt]; [shapeNodePath addLineToPoint:pointrb];
    //4
    [shapeNodePath closePath];
    //5
    SKShapeNode *wonkyBlock = [SKShapeNode node]; wonkyBlock.path = shapeNodePath.CGPath;
    //6
    UIBezierPath *physicsBodyPath = [UIBezierPath bezierPath]; [physicsBodyPath moveToPoint:
                                                                CGPointSubtract(pointlb, CGPointMake(-2, -2))]; [physicsBodyPath addLineToPoint:
                                                                                                                 CGPointSubtract(pointlt, CGPointMake(-2, 2))]; [physicsBodyPath addLineToPoint:
                                                                                                                                                                 CGPointSubtract(pointrt, CGPointMake(2, 2))]; [physicsBodyPath addLineToPoint:
                                                                                                                                                                                                                CGPointSubtract(pointrb, CGPointMake(2, -2))]; [physicsBodyPath closePath];
    //7
    wonkyBlock.physicsBody = [SKPhysicsBody
                              bodyWithPolygonFromPath:physicsBodyPath.CGPath]; wonkyBlock.physicsBody.categoryBitMask =
    CNPhysicsCategoryBlock; wonkyBlock.physicsBody.collisionBitMask =
    CNPhysicsCategoryBlock | CNPhysicsCategoryCat | CNPhysicsCategoryEdge;
    //8
    wonkyBlock.lineWidth = 1.0; wonkyBlock.fillColor =
    [SKColor colorWithRed:0.75 green:0.75 blue:1.0 alpha:1.0]; wonkyBlock.strokeColor =
    [SKColor colorWithRed:0.15 green:0.15 blue:0.0 alpha:1.0]; wonkyBlock.glowWidth = 1.0;
    return wonkyBlock;
}
